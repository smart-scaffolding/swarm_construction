...
class Block:
    ...
    def __init__(
        self, pipeline, position, block_id, block_file_location, color, display=False
    ):
    ...
    def update(
        ...
        pipeline,
        transform,
        color=VtkPipeline.vtk_named_colors(["Purple"]),
        show=True,
    ):
        if show:
            self.display(True)
    ...
    def remove(self, pipeline):
        pipeline.ren.RemoveActor(self.rendered_block)
