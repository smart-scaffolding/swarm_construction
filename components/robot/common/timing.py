...
class BehaviorTiming:
    ...
    def update_time(self, behavior, parent=None):
        ...
        # config.pusher.trigger("robot", "state", {"behavior": behavior, "parent": parent})
        # print("Updated time")
    ...
    def run(self):
        for dict_to_write in iter(self.data_queue.get, None):
            with open(self.filename, "a", newline="") as csvfile:
                ...
                # print("Wrote to file")
            # config.pusher.trigger("robot", "state", json.dumps(dict_to_write,default=default))
...
if __name__ == "__main__":
    ...
    behaviors = ["wait", "update", "move", "ferry", "build"]

    timer = BehaviorTiming(
        behaviors=behaviors, filename="../results/test.csv", write_data_interval=5
    )
    timer.start_behavior_timing()
    while True:
        rand_index = randint(0, len(behaviors) - 1)
        timer.update_time(behaviors[rand_index])
        sleep(randint(0, 5))
