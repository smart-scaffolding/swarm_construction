    ...
    def update(self):
        ...
        if len(self.blocks_to_move) <= 0:
            ...
            response_message = StatusUpdateMessage(
                status=self.status_identifier,
                payload="All blocks have been moved " "successfully",
            )
            ...
            self.communicator.send_communication(
                ...
                message=StatusUpdateMessage(
                    status=RobotBehaviors.WAIT,
                    payload=FerryBlocksStatusFinished(
                        blocks_moved=blocks_robot_has_moved
                    ),
                ),
            )
        ...
        if self.state.get(name=self.keys["block_placed_state"]) is False:
            ...
            response_message = StatusUpdateMessage(
                status=self.status_identifier, payload="Moving to place block"
            )
            self.communicator.send_communication(
                topic=self.robot_id, message=response_message
            )
            ...
            response_message = StatusUpdateMessage(
                status=self.status_identifier,
                payload="Block placed, getting next " "block to place",
            )
            self.communicator.send_communication(
                topic=self.robot_id, message=response_message
            )
            ...
            logger.debug(
                f"[{self.name.upper()}]: New block being moved: {self.move_block}"
            )
            ...
            self.blackboard.set(
                name=self.keys["navigation_second_key"],
                value=self.move_block.next_destination,
            )  # TODO: Change
            # to next destination of block

            print("Just set navigation first key")
            self.blackboard.set(
                name=self.keys["place_block_key"], value=self.move_block.next_destination,
            )
...
def create_move_blocks_root(
    ...
    motion_planning_behaviors = get_motion_planning_behaviors_tree(
        robot_communicator=robot_communicator,
        simulator_communicator=simulator_communicator,
        robot=robot,
        blueprint=blueprint,
