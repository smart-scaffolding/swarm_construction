...
from components.robot.communication.messages import MessageWrapper
# import threading
import zmq
import sys
import math
...
class ReceiveTopicThread(threading.Thread):
    ...
    def run(self):
        while not self.stoprequest.isSet():
            ...
            [topic, message] = self.socket.recv()
...
class HeartBeatTest:
    ...
    def get_communication(self):
        print(self.receive_messages_queue.empty())
        messages = []
    ...
    def heartbeat(self):
        while True:
            ...
            if self.get_communication():
                print("Received communication, sending heartbeat")
                self.send_communication(b"", "HI from robot")
            time.sleep(1)
...
if __name__ == '__main__':
    ...
    # loop.start()
    queue = Queue()
    hb = HeartBeatTest(receive_messages_socket='tcp://127.0.0.1:5555', receive_topics=b"",
                       send_messages_socket='tcp://127.0.0.1:5556', send_topics=b"")
